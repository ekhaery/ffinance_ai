  ...
  return (
    <nav className="bg-white border-b border-gray-200">
      ...
      <div className="max-w-4xl mx-auto px-4 flex items-center h-14 gap-1">
          ...
          <div className="relative" ref={settingRef}>
            ...
            {settingOpen && (
              <div className="absolute left-0 top-full mt-1 w-44 bg-white rounded-lg border border-gray-200 shadow-lg py-1 z-50">
                <Link href="/templates" onClick={() => setSettingOpen(false)} className={`block px-4 py-2 text-sm ${pathname.startsWith('/templates') ? 'text-blue-700 bg-blue-50' : 'text-gray-700 hover:bg-gray-50'}`}>Templates</Link>
                <Link href="/settings/categories" onClick={() => setSettingOpen(false)} className={`block px-4 py-2 text-sm ${pathname.startsWith('/settings/categories') ? 'text-blue-700 bg-blue-50' : 'text-gray-700 hover:bg-gray-50'}`}>Category</Link>
                <Link href="/settings/subcategories" onClick={() => setSettingOpen(false)} className={`block px-4 py-2 text-sm ${pathname.startsWith('/settings/subcategories') ? 'text-blue-700 bg-blue-50' : 'text-gray-700 hover:bg-gray-50'}`}>Subcategory</Link>
                <Link href="/settings/accounts" onClick={() => setSettingOpen(false)} className={`block px-4 py-2 text-sm ${pathname.startsWith('/settings/accounts') ? 'text-blue-700 bg-blue-50' : 'text-gray-700 hover:bg-gray-50'}`}>Account</Link>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Mobile hamburger */}
        <button className="md:hidden ml-auto p-2 rounded-md text-gray-600 hover:bg-gray-100" onClick={() => setMobileOpen((o) => !o)} aria-label="Toggle menu">
          {mobileOpen ? (
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white px-4 py-3 space-y-1">
          ...
          <div className="pt-1 border-t border-gray-100">
            ...
            <Link href="/templates" className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors ${pathname.startsWith('/templates') ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`}>Templates</Link>
            <Link href="/settings/categories" className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors ${pathname.startsWith('/settings/categories') ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`}>Category</Link>
            <Link href="/settings/subcategories" className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors ${pathname.startsWith('/settings/subcategories') ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`}>Subcategory</Link>
            <Link href="/settings/accounts" className={`block px-3 py-2 rounded-md text-sm font-medium transition-colors ${pathname.startsWith('/settings/accounts') ? 'bg-blue-50 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`}>Account</Link>
          </div>
        </div>
      )}
    </nav>
