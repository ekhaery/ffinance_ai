...
function ExpenseForm({ subcategories, accounts }: { subcategories: Subcategory[]; accounts: Account[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      {success && <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">Expense saved successfully!</div>}
          ...
          {nameOpen && nameSuggestions.length > 0 && (
            <ul className="absolute z-30 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-52 overflow-y-auto text-sm">
              {nameSuggestions.map((name) => (
                <li key={name} onMouseDown={() => autofill(name)} className="flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-blue-50 hover:text-blue-700 text-gray-700">
        ...
        <div className="flex flex-wrap gap-2">
          {accounts.map((a) => (
            <button key={a.id} type="button" onClick={() => setForm((prev) => ({ ...prev, account_id: prev.account_id === String(a.id) ? '' : String(a.id) }))}
              className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${form.account_id === String(a.id) ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:border-blue-400 hover:text-blue-600'}`}>
              {a.name}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {FAMILY_MEMBERS.map((m) => (
            <button key={m} type="button" onClick={() => setForm((prev) => ({ ...prev, family_member: prev.family_member === m ? '' : m }))}
              className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${form.family_member === m ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:border-blue-400 hover:text-blue-600'}`}>
              {m}
            </button>
          ))}
        </div>
...
function TransferForm({ accounts, balances }: { accounts: Account[]; balances: Record<number, number> }) {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-6">
      {success && <div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-700">Transfer berhasil!</div>}
      ...
      {/* From Account */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">From Account <span className="text-red-500">*</span></label>
        <div className="flex flex-wrap gap-2">
          {accounts.map((a) => (
      ...
      {/* To Account */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">To Account <span className="text-red-500">*</span></label>
        <div className="flex flex-wrap gap-2">
          {accounts.map((a) => (
            <button key={a.id} type="button" onClick={() => setToId(toId === a.id ? null : a.id)}
              className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-colors ${toId === a.id ? 'bg-green-600 border-green-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:border-green-400 hover:text-green-600'}`}>
              {a.name}
            </button>
          ))}
        </div>
      ...
      {fromId && toId && fromId !== toId && Number(amount) > 0 && (
        <div className="rounded-lg bg-gray-50 border border-gray-200 px-4 py-3 text-sm space-y-1">
          ...
          <div className="flex justify-between text-gray-600">
            ...
            <span className="text-red-600 font-medium">{(localBalances[fromId] ?? 0).toLocaleString('id-ID')} → {((localBalances[fromId] ?? 0) - Number(amount)).toLocaleString('id-ID')}</span>
          </div>
          <div className="flex justify-between text-gray-600">
            <span>{accounts.find(a => a.id === toId)?.name}</span>
            <span className="text-green-600 font-medium">{(localBalances[toId] ?? 0).toLocaleString('id-ID')} → {((localBalances[toId] ?? 0) + Number(amount)).toLocaleString('id-ID')}</span>
          </div>
        </div>
      )}
