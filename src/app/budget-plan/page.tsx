...
type Tab = 'monthly_template' | 'budget_plan'
...
// ─── Monthly Template tab ────────────────────────────────────────────────────

function MonthlyTemplateTab() {
    ...
    <>
      <div className="flex items-center justify-between mb-4">
        ...
        <Link
          ...
          className="rounded-lg bg-[#121358] px-3 py-2 text-sm font-medium text-white hover:bg-[#6668a8] transition-colors"
        >
          + New
        </Link>
      </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {templates.map((t) => (
                  <tr key={t.id} onClick={() => router.push(`/templates/${t.id}`)} className="hover:bg-gray-50 transition-colors cursor-pointer">
                    ...
                    <td className="px-5 py-4 text-right text-gray-700">{totalBudget(t).toLocaleString('id-ID')}</td>
                    <td className="px-5 py-4 text-gray-500">{new Date(t.created_at).toLocaleDateString('id-ID')}</td>
                    <td className="px-5 py-4 text-right space-x-5">
                      <button onClick={(e) => { e.stopPropagation(); setCopyTarget(t) }} className="text-[#121358] hover:opacity-70"><i className="fa-solid fa-copy" /></button>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleDelete(t.id) }} disabled={deleting === t.id} className="text-[#FA6781] hover:underline disabled:opacity-40">
                        <i className={`fa-solid fa-trash ${deleting === t.id ? 'opacity-40' : ''}`} />
                      </button>
                    </td>
                  </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="flex gap-3 pt-1">
              <button onClick={() => handleCopy(copyTarget)} disabled={copying}
...
export default function BudgetPlanPage() {
  const [activeTab, setActiveTab] = useState<Tab>('monthly_template')

  const tabs: { key: Tab; label: string }[] = [
    { key: 'monthly_template', label: 'Monthly Template' },
  ...
  return (
    <main className="min-h-screen bg-[#ffffff] px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-xl font-semibold text-gray-900 mb-6">Budget Plan</h1>
        ...
        {activeTab === 'monthly_template' && <MonthlyTemplateTab />}

        {activeTab === 'budget_plan' && (
          <div className="rounded-xl border border-dashed border-gray-300 bg-white px-6 py-16 text-center">
            <p className="text-gray-400 text-sm">Budget Plan — coming soon.</p>
          </div>
        )}
      </div>
    </main>
