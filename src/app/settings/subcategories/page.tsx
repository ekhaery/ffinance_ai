          ...
          <form onSubmit={handleCreate} className="flex gap-2 items-start">
            <div className="w-44">
              <select
                ...
                className={`w-full rounded-lg border px-3 py-2 text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500 ${errors.category ? 'border-red-400' : 'border-gray-300'}`}
              >
                ...
                {subcategories.map((sub) => (
                  <tr key={sub.id} className="hover:bg-gray-50">
                      ...
                      {editId === sub.id ? (
                        <div>
                          <select
                            ...
                            className={`rounded-lg border px-2 py-1 text-sm bg-white outline-none focus:ring-2 focus:ring-blue-500 ${editErrors.category ? 'border-red-400' : 'border-gray-300'}`}
                          >
                            <option value="">Select</option>
                            {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                          </select>
                          {editErrors.category && <p className="mt-0.5 text-xs text-red-500">{editErrors.category}</p>}
                        </div>
                      ) : (
                      ...
                      {editId === sub.id ? (
                        <div>
                          <input
                            ...
                            autoFocus
                            className={`rounded-lg border px-2 py-1 text-sm outline-none focus:ring-2 focus:ring-blue-500 ${editErrors.name ? 'border-red-400' : 'border-gray-300'}`}
                          />
                          {editErrors.name && <p className="mt-0.5 text-xs text-red-500">{editErrors.name}</p>}
                        </div>
                      ) : (
                    ...
                    <td className="px-5 py-3 text-right space-x-3">
                      {editId === sub.id ? (
                        <>
                          <button onClick={() => handleUpdate(sub.id)} disabled={saving} className="text-blue-600 hover:underline disabled:opacity-40">Save</button>
                        ...
                        <>
                          ...
                          <button onClick={() => handleDelete(sub.id)} disabled={deleting === sub.id} className="text-red-500 hover:underline disabled:opacity-40">
